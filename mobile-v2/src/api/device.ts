...
export interface DeviceAttribute { key: string; value: unknown; }
...
export function sendCommand(id: number, payload: Record<string, unknown>) {
  return request.post<{ code: number; data: { request_id: string; status: string } }>(
    `/devices/${id}/command`, payload
  );
}
export function createDevice(data: Partial<Device> & { mqtt_password?: string }) {
  return request.post<{ code: number; data: Device }>('/devices', data);
}
export function updateDevice(id: number, data: Partial<Device>) {
  return request.put<{ code: number; data: Device }>(`/devices/${id}`, data);
}
