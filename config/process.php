...
return [
    ...
    */
    ...
    | 通知发送进程
    |--------------------------------------------------------------------------
    | 从 Redis notify:queue 消费通知任务并发送（email/webhook/telegram 等）。
    | 独立进程，避免同步 HTTP/邮件发送阻塞告警引擎的事件循环。
    | 可按通知量增加进程数（多进程共享队列，rPop 天然不重复）。
    */
    'notification' => [
        'handler'  => app\process\NotificationProcess::class,
        'count'    => 1,
        'listen'   => '',
    ],
    ...
    */
    'crontab' => [
        'handler'  => app\process\CrontabProcess::class,
        'count'    => 1,
        'listen'   => '',
    ],
